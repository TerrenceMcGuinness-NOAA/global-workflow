...
async function listJobScripts() {
  try {
    // Navigate up from RUN directory to repository root and find jobs directory
    // RUN -> mcp_server_node -> Copilot -> utils -> scripts -> ci -> dev -> repo_root
    ...
    try {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Jobs directory not found at: ${jobsDir}`
          }
        ]
      };
    }
    ...
    if (jobScripts.length === 0) {
      return {
        content: [
          {
            type: 'text',
            text: 'No job scripts found in the jobs directory.'
          }
        ]
      };
    }

    // Group jobs by system type for better organization
    const gdas = jobScripts.filter(job => job.includes('GDAS'));
    const gfs = jobScripts.filter(job => job.includes('GFS') && !job.includes('GDAS'));
    const gefs = jobScripts.filter(job => job.includes('GEFS'));
    const global = jobScripts.filter(job => job.includes('GLOBAL'));
    const other = jobScripts.filter(job => 
      !job.includes('GDAS') && 
      !job.includes('GFS') && 
      !job.includes('GEFS') && 
      !job.includes('GLOBAL')
    ...
    let result = `Found ${jobScripts.length} job scripts in the jobs directory\n\n`;
    
    if (gdas.length > 0) {
      result += `GDAS Jobs (${gdas.length}):\n${gdas.map(job => `  - ${job}`).join('\n')}\n\n`;
    }
    
    if (gfs.length > 0) {
      result += `GFS Jobs (${gfs.length}):\n${gfs.map(job => `  - ${job}`).join('\n')}\n\n`;
    }
    
    if (gefs.length > 0) {
      result += `GEFS Jobs (${gefs.length}):\n${gefs.map(job => `  - ${job}`).join('\n')}\n\n`;
    }
    
    if (global.length > 0) {
      result += `Global Jobs (${global.length}):\n${global.map(job => `  - ${job}`).join('\n')}\n\n`;
    }
    
    if (other.length > 0) {
      result += `Other Jobs (${other.length}):\n${other.map(job => `  - ${job}`).join('\n')}\n\n`;
    }
