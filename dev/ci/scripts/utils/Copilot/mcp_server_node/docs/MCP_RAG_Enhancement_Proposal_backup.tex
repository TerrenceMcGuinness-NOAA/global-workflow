...
% Main navy header
\fill[noaaNavy] (current page.north west) rectangle ([yshift=-3cm]current page.north east);

% Blue accent stripe
\fill[noaaBlue] ([yshift=-2.8cm]current page.north west) rectangle ([yshift=-3.2cm]current page.north east);

% NOAA Logo simulation with enhanced styling
\node[white,font=\huge\bfseries] at ([xshift=2cm,yshift=-1.5cm]current page.north west) {NOAA};
\node[white,font=\large] at ([xshift=2cm,yshift=-2.2cm]current page.north west) {National Weather Service};

% Decorative technical elements
\foreach \i in {0,0.4,0.8,1.2,1.6} {
    \draw[noaaLightBlue,line width=3pt,opacity=0.8] ([xshift=15cm+\i*0.5cm,yshift=-0.5cm]current page.north west) -- ([xshift=15cm+\i*0.5cm,yshift=-2.5cm]current page.north west);
}

% Technical circuit-like pattern
\draw[noaaLightBlue,line width=1pt,opacity=0.6] ([xshift=12cm,yshift=-1cm]current page.north west) -- ([xshift=14cm,yshift=-1cm]current page.north west) -- ([xshift=14cm,yshift=-2cm]current page.north west) -- ([xshift=16cm,yshift=-2cm]current page.north west);

% Bottom footer with enhanced styling
\fill[noaaNavy] ([yshift=1.8cm]current page.south west) rectangle (current page.south east);
\fill[noaaBlue] ([yshift=1.6cm]current page.south west) rectangle ([yshift=1.8cm]current page.south east);
\node[white,font=\footnotesize] at ([yshift=0.9cm]current page.south) {Department of Commerce | National Oceanic and Atmospheric Administration | NOAA.gov};
\end{tikzpicture}
...
\begin{tikzpicture}
\draw[noaaBlue, line width=4pt] (0,0) -- (10,0);
\foreach \x in {0,2,4,6,8,10} {
    \fill[noaaNavy] (\x,-0.15) circle (0.08);
}
% Technical connection lines
\draw[noaaLightBlue, line width=1pt, opacity=0.7] (-1,-0.5) -- (1,-0.5) -- (1,0.5) -- (3,0.5);
\draw[noaaLightBlue, line width=1pt, opacity=0.7] (7,-0.5) -- (9,-0.5) -- (9,0.5) -- (11,0.5);
\end{tikzpicture}
\end{center}
\end{titlepage}
...
% Define simple box style
\tikzstyle{procbox} = [rectangle, draw=noaaNavy, fill=noaaLight, thick, 
                       text width=2.5cm, text centered, minimum height=1cm]

% Main process flow
\node[procbox] (input) at (0,0) {Documentation \\ Sources};
\node[procbox] (process) at (4,0) {RAG \\ Processing};
\node[procbox] (output) at (8,0) {Enhanced \\ Responses};

% Arrows
\draw[thick,->,color=noaaNavy] (input) -- (process);
\draw[thick,->,color=noaaNavy] (process) -- (output);

% Labels
\node[below,color=noaaBlue] at (2,-0.8) {\small Ingestion};
\node[below,color=noaaBlue] at (6,-0.8) {\small Generation};
\end{tikzpicture}
\end{center}
...
% Simple boxes for documentation sources
\tikzstyle{docbox} = [rectangle, draw=noaaBlue, fill=noaaBlue!10, thick, 
                      text width=4cm, text centered, minimum height=1.2cm, rounded corners=3pt]

\node[docbox] (ee2) at (0,2) {\textbf{Enterprise Environment 2} \\ (EE2) Standards};
\node[docbox] (rocoto) at (0,0) {\textbf{Rocoto Workflow} \\ Management};
\node[docbox] (rdhpcs) at (0,-2) {\textbf{NOAA RDHPCS} \\ Infrastructure};

% Arrow pointing to RAG system
\node[rectangle, draw=noaaNavy, fill=noaaLight, thick, text width=3cm, text centered, minimum height=2cm] 
      (rag) at (7,0) {\textbf{RAG System} \\ \\ Enhanced \\ Documentation \\ Access};

\draw[thick,->,color=noaaNavy] (ee2.east) -- (rag.west);
\draw[thick,->,color=noaaNavy] (rocoto.east) -- (rag.west);
\draw[thick,->,color=noaaNavy] (rdhpcs.east) -- (rag.west);
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
    secure/.style={rectangle, draw=red!70, fill=red!10, thick, minimum width=2.5cm, minimum height=1.2cm, text centered, rounded corners=2pt},
    cloud/.style={ellipse, draw=noaaBlue, fill=noaaBlue!15, thick, minimum width=3cm, minimum height=1.5cm, text centered},
    laptop/.style={rectangle, draw=orange!70, fill=orange!10, thick, minimum width=2cm, minimum height=1cm, text centered, rounded corners=2pt},
    arrow/.style={->, thick}
]

% GFE Laptops (Limited)
\node[laptop] (gfe1) at (0,2) {GFE\\Laptop 1};
\node[laptop] (gfe2) at (0,0) {GFE\\Laptop 2};
\node[laptop] (gfe3) at (0,-2) {GFE\\Laptop 3};

% Parallel Works Cloud
\node[cloud] (pw) at (5,0) {\textbf{Parallel Works}\\Isolated Clusters};

% HPC Systems (Secure)
\node[secure] (hera) at (10,1.5) {Hera\\HPC};
\node[secure] (orion) at (10,-1.5) {Orion\\HPC};

% Connections
\draw[arrow, noaaAccent, thick] (gfe1) -- node[above, font=\tiny] {Secure Access} (pw);
\draw[arrow, noaaAccent, thick] (gfe2) -- (pw);
\draw[arrow, noaaAccent, thick] (gfe3) -- (pw);

\draw[arrow, noaaNavy, thick] (pw) -- node[above, font=\tiny] {Controlled Deploy} (hera);
\draw[arrow, noaaNavy, thick] (pw) -- node[below, font=\tiny] {Secure Transfer} (orion);

% Security barriers
\draw[dashed, red!70, thick] (3,-3) -- (3,3);
\draw[dashed, red!70, thick] (7.5,-3) -- (7.5,3);

\node[red!70, font=\tiny] at (3,-3.3) {Security Boundary 1};
\node[red!70, font=\tiny] at (7.5,-3.3) {Security Boundary 2};
