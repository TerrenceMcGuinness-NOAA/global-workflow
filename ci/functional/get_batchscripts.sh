...
list_jobs=$(rocotostat -v 10 -d $EXPDIR/${PSLOT}.db -w $EXPDIR/${PSLOT}.xml | grep -Ev 'CYCLE|===' | awk '{print $2}')
...
for job in ${list_jobs}; do
  echo $job
  rm -f /tmp/temp.txt
  yes | rocotoboot -v 10 -d $EXPDIR/${PSLOT}.db -w $EXPDIR/${PSLOT}.xml -c ${SDATE}00 -t $job >& /tmp/temp.txt
